...
Supports multithreaded mode for handling multiple concurrent connections.
All requests are logged to a file in the TEMP directory (Interceptor_Log.txt).
...
Enables multithreaded mode to handle multiple concurrent connections. 
Each connection is processed in its own thread for better performance under load.
...
Interceptor.ps1 -Multithread
Interceptor.ps1 -Multithread -Tamper -ListenPort 8080
...
$script:LogFilePath = Join-Path $env:TEMP "Interceptor_Log.txt"

# Synchronized hashtable for thread-safe logging
$script:SyncHash = [hashtable]::Synchronized(@{})
$script:SyncHash.LogFile = $script:LogFilePath
...
function Write-InterceptorLog
{
    ...
    $mutex = New-Object System.Threading.Mutex($false, "InterceptorLogMutex")
    try
    {
        [void]$mutex.WaitOne()
...
function Receive-ServerHttpResponse ([System.Net.WebResponse] $response)
{
	...
	Try
	{
		...
		Write-InterceptorLog $rstring "White" 
...
function Receive-ClientHttpRequest([System.Net.Sockets.TcpClient] $client, [System.Net.WebProxy] $proxy)
{
	...
	{	
		...
		if($methodParse[0] -ceq "CONNECT")
		{
			...
			Write-InterceptorLog $SSLRequest "Yellow"
		...
		Else
		{
			Write-InterceptorLog $requestString "Cyan"
...
{	
	...
	# Initialize log file
	$logHeader = "`r`n========================================`r`n" + "Interceptor Started: " + [DateTime]::Now.ToString("yyyy-MM-dd HH:mm:ss") + "`r`n========================================`r`n"
	Add-Content -Path $script:LogFilePath -Value $logHeader
	Write-InterceptorLog ("Log file: " + $script:LogFilePath) "Green"
	...
	if($AutoProxyConfig)
	{
		...
		Write-InterceptorLog "Proxy is now enabled" "White"
	...
	if($ProxyServer)
	{
		...
		Write-InterceptorLog ("Using Proxy Server " + $ProxyServer + " : " + $ProxyPort) "White"
	}
	else
	{
		$proxy = $null
		# If you are going Direct.  You need this to be null, or HTTPWebrequest loops...
		Write-InterceptorLog "Using Direct Internet Connection" "White"
	}
	...
	if($Multithread)
	{
		Write-InterceptorLog ("Listening on " + $port + " (Multithreaded Mode)") "Green"
		...
		# Get current script content to pass to runspaces
		...
		while($true)
		{
			...
			if($client -ne $null)
			{
				...
				[void]$PowerShell.AddScript({
					param($clientObj, $proxyObj, $tamperFlag, $logPath, $scriptDef)
					
					# Execute the script to load all functions
					Invoke-Expression $scriptDef
					...
					try
					{
						Receive-ClientHttpRequest $clientObj $proxyObj
					}
					catch
					{
						Write-InterceptorLog ("Thread Error: " + $_.Exception.Message) "Red"
					}
				})
				[void]$PowerShell.AddArgument($client)
				[void]$PowerShell.AddArgument($proxy)
				[void]$PowerShell.AddArgument($Tamper)
				[void]$PowerShell.AddArgument($script:LogFilePath)
				[void]$PowerShell.AddArgument($scriptContent)
				...
				{
					$CompletedJob.PowerShell.EndInvoke($CompletedJob.Job)
	...
	{
		Write-InterceptorLog ("Listening on " + $port + " (Single-threaded Mode)") "White"
