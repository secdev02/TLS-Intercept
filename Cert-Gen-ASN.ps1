...
if ($ParseOnly -and $ParseFile) {
    ...
    Write-Host ("Parsing file: " + $ParseFile) -ForegroundColor Yellow
    Write-Host ""
    
    $certBytes = [System.IO.File]::ReadAllBytes($ParseFile)
    Write-Host ("Total file size: " + $certBytes.Length + " bytes") -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "`n=== Parsing Generated Certificate ===" -ForegroundColor Cyan
    Write-Host ""
    
    $certBytes = [System.IO.File]::ReadAllBytes($cerPath)
    Write-Host ("Total certificate size: " + $certBytes.Length + " bytes") -ForegroundColor Gray
    Write-Host ""
    Write-Host "=== ASN.1 Structure ===" -ForegroundColor Cyan
    Write-Host ""
